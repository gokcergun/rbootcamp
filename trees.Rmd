...
```{r}
unique(df_trees_hasdate[grep(df_trees_hasdate$species_lat, pattern = "^Corymbia", ignore.case = TRUE), 'species_lat'])
```


```{r}
unique(df_trees_hasdate[grep(df_trees_hasdate$species_lat, pattern = "^Pittosporum", ignore.case = TRUE), 'species_lat'])
```
...
We’ll also create a few more columns that will help us display labels in the plot.

```{r}
#### code from website
df_trees_hasdate_formated <- df_trees_hasdate %>%
  group_by(species_lat) %>%
