...
```{r}
species_cumsum <- ggplot(df_top5_cumsum, aes(x=year,y=csum, group=species_first, col=factor(species_first))) + geom_line() + theme_minimal()
```

```{r}
anim2 <- species_cumsum + transition_states(year, transition_length = 4, state_length = 1) + view_follow(fixed_x = TRUE)  +
  labs(title = 'cumulative total of top 5 species over time: {closest_state}',  
...
```
```{r}
df_top5_cumsum$year <- as.Date(df_top5_cumsum$year)
```
